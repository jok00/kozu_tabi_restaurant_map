...
店舗メモは入力停止から少し待つと自動保存されます。`★ お気に入り` フィルターを選ぶと、その地域でお気に入りに登録されている店舗だけにカード一覧と地図上のピンを絞ります。
...
座標は POI 由来を優先します。住所ジオコーディングや住所近傍から推定した座標は、`map_position.correction_note` と `notes.coordinate_status` にその旨を記録します。推定座標を使う店舗は、カードの営業時間などと同じサマリー行に `ピンの位置は推定` と表示します。
